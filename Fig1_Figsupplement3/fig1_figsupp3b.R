...
mpath <- ""
fname <- paste("modern.gwsig", sep="")

setwd(mpath)
f <- read.table(paste(fname, sep=""))

prefixes <- c("giant", "nealelab", "poru", "all.nopc","all.pc", "brit.nopc", "brit.pc")
...
df <- rbind(giant, nealelab, poru, allnopc, allpc, britnopc, britpc)
names(df) <- c("population", "score", "cilo", "cihi", "GWAS")

## reorder factors for plotting
orderp <- c(1,2,4,3)
df$population <- factor(df$population, levels = df$population[rep(orderp,7)])

### plotting
p <- ggplot(df,aes(x=population,y=score)) +
  facet_wrap( ~ GWAS, ncol=4) +
...
ggsave(p, file=paste("fig1_figsupp3b.pdf",sep=""),width=11.5, height=6.5,dpi=300)
